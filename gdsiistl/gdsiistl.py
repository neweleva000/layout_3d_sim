...
layerstack = {
    ...
    0: (-0.01, 0.00, 'gnd-PEC priority=5'),
    1: (0.00, 0.1, 'substrate-epsilon=4'),
    2: (0.1, 0.11, 'sig-PEL priority=10'),
    3: (-0.01, 0.11, 'port z 1'),
    4: (-0.01, 0.11, 'port z 2'),
}
...
for layer_number, polygons in layers.items():
    ...
    for index, (polygon, _, _) in enumerate(polygons):
        ...
        #TODO seems to work for my use case
        #delta = 0.1 # inset each vertex by this much (smaller has broken one file)
        delta = 0
...
for layer in layers:
    ...
    gds_dir = os.path.dirname(gdsii_file_path)
    filename = gds_dir + '/{}.stl'.format(layername)
